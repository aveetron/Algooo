...
public class Bisect
{
    ...
    while (n <= maxIts && error > tol)
        ...
        System.out.format (
                 "%10s%10s%10s%10s%10s\n",
                   n, Math.round(xnew * factor) / factor,
                   Math.round(h(x) * factor) / factor,
                   Math.round(dx * factor) / factor ,
                   Math.round(f(x) * factor) / factor ,
                   Math.round(hprime(x) * factor) / factor);
        n++;
